      ...
      >
        ...
        >
          <div className="px-6 py-5 border-b border-[var(--border)] bg-[var(--bg-subtle)]">
            <h2 className="text-lg font-light text-[var(--text-h)] display-font">{t('dayValidation.title')}</h2>
          </div>

          <div className="px-6 py-6 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--text)]">{t('dayValidation.recordedMeals')}</span>
              <span className="text-lg font-medium text-[var(--text-h)] tabular-nums">{mealCount}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--text)]">{t('dayValidation.totalCalories')}</span>
              <span className="text-lg font-medium text-[var(--text-h)] tabular-nums">{totalKcal} kcal</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--text)]">{t('dayValidation.dailyGoal')}</span>
              <span className="text-lg font-medium text-[var(--text-h)] tabular-nums">{dailyGoals.calories} kcal</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--text)]">{t('dayValidation.coverage')}</span>
