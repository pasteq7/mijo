...
export function AdvancedTooltip({ food, isVisible }: Props) {
  ...
  const grouped = GROUP_ORDER
    .map((group) => ({
      group,
      label: t('goalsModal.nutrientGroups.' + group),
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-semibold mb-1">
        {food.emoji} {t('foods.' + food.id)}
      </div>
      <div className="text-[10px] text-[var(--text)] mb-3">{t('common.per100')} 100 {food.unit}</div>
          ...
          <div key={g.group}>
            ...
            <div className="space-y-1">
              {g.nutrients.map((n) => (
                ...
                >
                  <span className="text-[var(--text)]">{t('nutrients.' + n.id + '.label')}</span>
