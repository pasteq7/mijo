  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-12 pb-24">
      <header className="mb-12 flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-light text-[--text-h] tracking-tight display-font">Mon Repas</h1>
          <div className="h-1 w-16 bg-[--accent] mt-4 rounded-full opacity-60" />
        </div>
        {selectedFoods.length > 0 && (
          <motion.button
            ...
            onClick={onSaveMeal}
            className="flex items-center gap-2 px-6 py-3 bg-[--accent] text-white rounded-2xl font-medium hover:bg-[#5C7D5B] transition-colors shadow-[var(--shadow-sm)]"
          >
      ...
      <div className="flex items-center gap-4 p-5 bg-[--bg-subtle] border border-[--border] rounded-2xl text-[--text-h] text-sm">
        <div className="p-2.5 bg-white shadow-[var(--shadow-sm)] rounded-xl text-[--info]"><Pill size={20} strokeWidth={1.5} /></div>
        <div>
          <span className="font-medium block text-[--text-h] mb-0.5">Rappel Quotidien : Compléments Végans</span>
          <span className="text-[--text] text-xs">La B12 et la Vitamine D se prennent en supplémentation. Prends-tu tes vitamines ?</span>
        </div>
      </div>
        ...
        <div className="space-y-8">
          <div className="flex flex-col gap-6">
            <div className="flex justify-center mb-4">
              <div className="bg-[--warm-100] p-1.5 rounded-2xl flex gap-1 shadow-[var(--shadow-sm)]">
                <button
                  onClick={() => setViewMode('meal')}
                  className={clsx(
                    "px-6 py-2 rounded-xl text-xs font-medium transition-all relative",
                    viewMode === 'meal' ? "text-[--text-h]" : "text-[--text] hover:text-[--text-h]"
                  )}
                >
                ...
                <button
                  ...
                  className={clsx(
                    "px-6 py-2 rounded-xl text-xs font-medium transition-all relative",
                    viewMode === 'day' ? "text-[--text-h]" : "text-[--text] hover:text-[--text-h]"
                  )}
                >
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {viewMode === 'meal' ? (
                  ...
                  <DayStats totals={totals} goals={dailyGoals} />
                )}
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-end border-b border-[--border] pb-4">
              <h3 className="text-xs font-semibold text-[--text] uppercase tracking-[0.1em]">
                Couverture nutritionnelle ({viewMode === 'meal' ? 'Repas' : 'Journée'})
              </h3>
              {viewMode === 'meal' && hasHistoryAdjustment && (
                <span className="text-[10px] bg-[--highlight] text-white px-2.5 py-1 rounded-full font-medium shadow-[var(--shadow-sm)]">
